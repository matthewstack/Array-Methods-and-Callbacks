...
// receive data (fifaData), getFinalsCB, return array 
function getYears(data, getFinalsCB) {
    const theFinalsYears = getFinalsCB(data);
    const finalsYears = theFinalsYears.map(function(item){
        return item.Year;

    });
   return finalsYears; // map through finals callsback to get all of the years (item.Year) ---- return array
}
...
function getWinnersByYear(data, getYearsCB, getWinnersCB) {
    const theWinners = getWinnersCB(data);
    const theYears = getYearsCB(data);

    const yearsAndWinners = theWinners.map(function (value, i){
        return `In ${theYears[i]}, ${theWinners[i]} won the world cup!`;
    });
...
function getAverageGoals(getFinalsCB) {
    const numberOfGames = getFinalsCB.length 
    const homeTeamGoals = getFinalsCB.reduce(function(acc, item){
       return acc + item['Home Team Goals'];
    },0)

    const awayTeamGoals = getFinalsCB.reduce(function(acc, item){
        return acc + item['Away Team Goals'];   
    },0)
    
    const avgGoals = (homeTeamGoals + awayTeamGoals) / numberOfGames
